...
<div id="myDIV">
<div>
<input type="text" placeholder="Search..." id="txtSearch" autocomplete="off"> (More than 1000 courses available, please use Search...)
</div>
<div id="courses">
...
$objCourses = $DB->get_records_sql("select a.id,a.fullname,b.name as catname from {$CFG->prefix}course a,{$CFG->prefix}course_categories b where a.visible = 1 and a.id not in ($objSelectedCourses->selected_courses) and a.category = b.id limit 1,5");
	...
	$objCourses = $DB->get_records_sql("select a.id,a.fullname,b.name as catname from {$CFG->prefix}course a,{$CFG->prefix}course_categories b  where a.visible = 1 and a.category = b.id limit 1,5");
}
} else {
if($objSelectedCourses->selected_courses != ''){
...
$objCourses = $DB->get_records_sql("select a.id,a.fullname,b.name as catname from {$CFG->prefix}course a,{$CFG->prefix}course_categories b where a.visible = 1 and a.id not in ($objSelectedCourses->selected_courses) and a.category = b.id and a.cm_bu_id = $USER->cm_bu_id limit 1,5");
...
/*
function fnAjaxCall(argCourseId)
	var vLPId = $('#hidLPId').val();
    ...
    if (!this.checked) {
	    ...
	    $.ajax({
            url: './ajax/ajax_update_courses.php',
            type: 'post',
            data: {lp_id:vLPId,lp_checked:0,lp_courseid:argCourseId},
            success:function(response){                              
				//$("#courses").html(response);                 
            }
	    ...
	    $.ajax({
            url: './ajax/ajax_update_courses.php',
            type: 'post',
            data: {lp_id:vLPId,lp_checked:1,lp_courseid:vCourse},
            success:function(response){                              
				//$("#courses").html(response);                 
            }
...
if($(CheckboxId).prop("checked")){
  ...
  $.ajax({
            url: './ajax/ajax_update_courses.php',
            type: 'post',
            data: {lp_id:vLPId,lp_checked:lp_checked,lp_courseid:vCourseId},
            success:function(response){                              
  ...
  //$(document).ready(function() { lity-opened
...
//$('[data-lity-close]', parent.location.reload(true)).click();		
</script>
<?php 
