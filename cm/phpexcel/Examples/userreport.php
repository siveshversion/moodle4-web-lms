...
curl_setopt($ch, CURLOPT_URL, $CFG->wwwroot."/cm/api/methods.php?methodname=userCourseReport");
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D1', 'Business Unit')
			->setCellValue('E1', 'Enrolled')
			->setCellValue('F1', 'Completed')
			->setCellValue('G1', 'In Progress')
			->setCellValue('H1', 'Not Started');
...
foreach($arrData as $transaction){
	...
	$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('B'.$row, $transaction['user_name'])
			->setCellValue('C'.$row, $transaction['user_fullname'])
			->setCellValue('D'.$row, $transaction['bu_name'])
			->setCellValue('E'.$row, $transaction['enrolled_cnt'])
			->setCellValue('F'.$row, $transaction['completed_cnt'])
			->setCellValue('G'.$row, $transaction['inprogress_cnt'])
			->setCellValue('H'.$row, $transaction['notstarted_cnt']);
			$row++;
			$sNo++;
