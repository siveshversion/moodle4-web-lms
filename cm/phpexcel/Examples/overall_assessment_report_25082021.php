...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('F1', 'Time taken (Hrs)')
			->setCellValue('G1', 'No of Attempt')
			->setCellValue('H1', 'No of correct answers')
...
foreach($course_result as $attempt){
	/*echo '<pre>';
	print_r($attempt);
	echo '</pre>';*/
      ...
      $params = array( 'quizid' => $vQuizId,'userid' => $attempt->user_id);
       ...
       $serverurl = $CFG->wwwroot . '/webservice/rest/server.php'. '?wstoken=' . $CFG->moodletoken . '&wsfunction='.$functionnasme;      
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('B'.$row, $attempt->firstname)
			->setCellValue('C'.$row, $attempt->lastname)
			->setCellValue('D'.$row, $attempt->state)
            ->setCellValue('E'.$row, $vCompletedOn)
			->setCellValue('F'.$row, $vTimeTaken)
			->setCellValue('G'.$row, $vTotalAttempt)
			->setCellValue('H'.$row, $vCorrectAnswer .' / '.$vTotalQuestions)
			->setCellValue('I'.$row, $vGrade);
