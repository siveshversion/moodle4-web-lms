...
$objPHPExcel->setActiveSheetIndex(0)
  ...
  if(is_siteadmin()){
	...
	$categorytypes =	$DB->get_records_sql("SELECT c.id,c.fullname,c.shortname,c.summary,cc.id as catid,cc.name,cc.parent,c.courselevel,c.coursetype,c.points,c.hours,c.mints FROM mdl_course c join mdl_course_categories cc on c.category = cc.id where c.coursetype != 3 and c.visible = 1 and c.id > 1");
  } else {
	  global $USER;
...
$allcids = implode(',',$sub2);
 ...
 if(!empty($allcids)){
	  $categorytypes =	$DB->get_records_sql("SELECT c.id,c.fullname,c.shortname,c.summary,cc.id as catid,cc.name,cc.parent,c.courselevel,c.coursetype,c.points,c.hours,c.mints FROM mdl_course c join mdl_course_categories cc on c.category = cc.id where c.coursetype != 3 and cc.id IN($allcids) and c.tires IN($t) and c.visible = 1 and c.id > 1");
 }
