...
$allcids = implode(',',$sub2);
 ...
 if(!empty($allcids)){
		   $query = "select a.id,c.id as courseid,c.fullname as coursename,a.userid,a.timestart from {$CFG->prefix}user_enrolments a,{$CFG->prefix}enrol b,{$CFG->prefix}course c,{$CFG->prefix}course_categories ccat where a.enrolid = b.id and b.courseid = c.id and c.category = ccat.id and ccat.id IN($allcids) $vWhere";
